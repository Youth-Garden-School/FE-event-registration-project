...
export async function getMyRegistration(
  ...
  // POST /registrations/me với body { page:0, size:10, sortBy:'createdAt' }
  const res = await getUserRegistrations(0, 10, "createdAt")
  const regs = res.data.result.content
  return regs.find((r) => r.event.id === eventId) ?? null
}
    ...
    <div className="space-y-0">
      {Object.entries(eventsByDate).map(([date, dateEvents], index, array) => (
        <div key={date} className="relative">
          <div className="grid grid-cols-[120px_30px_1fr] gap-0">
            {/* Cột bên trái: Ngày và thứ */}
            <div className="pt-6">
              <div className="font-medium">{dateEvents[0].dateLabel}</div>
              <div className="text-sm text-gray-400">{dateEvents[0].dayLabel}</div>
            </div>
            ...
            <div className="relative flex justify-center">
              {/* Đường kẻ dọc */}
              <div
                className={`absolute w-px bg-white/10 left-1/2 transform -translate-x-1/2 ${
                  index === 0 ? "top-7" : "top-0"

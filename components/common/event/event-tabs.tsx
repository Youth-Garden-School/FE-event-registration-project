...
export default function EventTabs() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        // 2) Chuyển thành UI với flags cho quản lý vs đã đăng ký
        const allUI = regs.map((r) => {
          ...
          return buildEventUI(r, attendees, attendeeCount, isUserEvent)
        })
        ...
        ups.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime())
        pst.sort((a, b) => new Date(b.endTime).getTime() - new Date(a.endTime).getTime())
  ...
  if (loading) return (
      <div className=" bg-white-50 flex items-center justify-center">
        <div className="bg-white p-8 rounded-xl shadow-lg text-center">
          ...
          <p className="text-gray-600 font-medium">Đang tải sự kiện của bạn...</p>
        </div>
      </div>
    )
  ...
  const pagedUpcoming = upcomingEvents.slice((upcomingPage - 1) * EVENTS_PER_PAGE, upcomingPage * EVENTS_PER_PAGE)
  const pagedPast = pastEvents.slice((pastPage - 1) * EVENTS_PER_PAGE, pastPage * EVENTS_PER_PAGE)

  return (
    <>  
      <Tabs defaultValue="upcoming" className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-gray-900 text-[32px] font-semibold">Sự kiện của bạn</h1>
        ...
        <TabsContent value="upcoming">
          {pagedUpcoming.length > 0 ? (
            ...
            <NoEventsComponent message="Không có sự kiện sắp tới" subMessage="Bạn chưa đăng ký sự kiện nào." />
          )}
        </TabsContent>
        ...
        <TabsContent value="past">
          {pagedPast.length > 0 ? (
            ...
            <NoEventsComponent message="Không có sự kiện đã qua" subMessage="Bạn chưa có lịch sử sự kiện." />
          )}
        </TabsContent>
      </Tabs>

      {selectedEvent && (
        <EventModal isOpen={isModalOpen} onClose={closeModal} event={selectedEvent} onCancel={handleRemoveEvent} />
      )}
    </>
  )
}
